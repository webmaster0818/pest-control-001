...
# ページデータ読み込み
with open('scripts/pest-data.json', 'r', encoding='utf-8') as f:
...
# テンプレート生成関数
def generate_page(pest_key, pest_info):
    name = pest_info['name']
    name_en = pest_info['nameEn']
    title = pest_info['title']
    description = pest_info['description']
    price_range = pest_info['priceRange']
    icon = pest_info['icon']
...
export const metadata: Metadata = {{
  ...
  openGraph: {{
    title: "{title}",
    description: "{description}",
    url: "https://pest-control-001.pages.dev/{name_en}",
  }},
}};

export default function {name.capitalize()}Page() {{
{common_companies}
    ...
    <div className="min-h-screen bg-base-200">
      ...
      <section className="py-8 px-4 bg-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-lg text-gray-700">
            {name}の特徴・被害状況から駆除方法まで、適切な対処法をご紹介します
          </p>
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-base-100">
        <div className="max-w-7xl mx-auto">
          <h2 className="heading-primary text-center mb-8">{name}駆除業者の比較</h2>
                    ...
                    <div className="space-y-3">
                      <div className="bg-base-200 p-4 rounded-lg">
                        ...
                        <p className="text-gray-300 text-sm">
                          迅速に対応していただき、{name}の被害がすぐに解決しました。料金も明確で安心できました。
                        </p>
                      </div>
                      <div className="bg-base-200 p-4 rounded-lg">
                        <div className="flex items-center gap-2 mb-2">
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          <h2 className="heading-primary text-center mb-8">{name}駆除の料金相場</h2>
            ...
            <table className="table w-full bg-white text-black">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="bg-white text-black border border-gray-300">{price_range}</td>
                </tr>
                <tr>
      ...
      <section className="py-16 px-4 bg-green-500 text-white">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-4">{name}駆除のご相談は今すぐ無料で</h2>
          <p className="text-xl mb-8">
            専門業者があなたの{name}被害を解決します
          </p>
...
# 各ページ生成
for pest_key, pest_info in pest_data.items():
    name_en = pest_info['nameEn']
    os.makedirs(f'app/{name_en}', exist_ok=True)
    
    page_content = generate_page(pest_key, pest_info)
    
    with open(f'app/{name_en}/page.tsx', 'w', encoding='utf-8') as f:
        f.write(page_content)
    
    print(f'✓ {pest_info["name"]}ページを生成しました')
