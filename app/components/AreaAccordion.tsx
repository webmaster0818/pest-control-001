  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {areas.map((area, index) => (
        <div key={area.name} className="border-2 border-gray-200 rounded-lg overflow-hidden bg-white">
          <div
            className={`flex items-center justify-between p-4 cursor-pointer hover:bg-blue-50 transition-all ${
              area.districts ? "" : "pointer-events-none"
            }`}
            onClick={() => area.districts && toggleAccordion(index)}
          >
            <Link href={area.href} className="text-lg font-semibold text-gray-900 hover:text-blue-600 flex-1">
              {area.name}
            </Link>
            {area.districts && (
              <svg
